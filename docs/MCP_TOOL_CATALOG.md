...
| `wp-auto/posts-search` | `wp-auto-posts-search` | Read-only | `read` plus per-object visibility enforcement | Implemented |
| `wp-auto/post-get` | `wp-auto-post-get` | Read-only | `read_post` for the target object | Implemented |
