...
WePuu Auto Connector needs a standards-compliant direct MCP server. WordPress 6.9 includes Abilities API in core. The official `WordPress/mcp-adapter` project maps WordPress abilities to MCP tools/resources/prompts and provides HTTP/STDIO transports.
...
3. The build must include the required runtime code for a standalone WePuu Auto Connector installation while MCP Adapter remains unavailable as a WordPress.org dependency.
