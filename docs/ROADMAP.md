...
6. **Phase 1.4.5 - Remote URL Import (implemented on `main`; exact-main validation complete):** implemented the open-world downloader last, with the frozen SSRF/DNS/redirect/time/byte/MIME policy and the exact eighteen-tool allowlist. Plugin Check remains an environment-gated release follow-up.
7. **Phase 1.4.6 - Integration/Security Validation (exact-main complete):** automated and live wp-env-equivalent/Streamable HTTP behavior, exact schemas/permissions/privacy/allowlist, ingestion concurrency, filesystem cleanup, SSRF matrix, state integrity, and two zero-finding security reviews all pass. Plugin Check remains an environment-gated follow-up before formal Phase 1.4 sealing.
