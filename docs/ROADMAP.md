...
3. **Phase 1.3.2.0 - modified_gmt Sentinel Compatibility Amendment (next):** documentation and contract checkpoint only; review the legitimate Core draft sentinel `0000-00-00 00:00:00` and define only the `expected_modified_gmt` validation compatibility needed before Update. No Update runtime implementation.
4. **Phase 1.3.2 - Draft Update + Best-effort Optimistic Concurrency:** after the 1.3.2.0 amendment is separately reviewed and frozen, implement and validate only `wp-auto/post-update` and `wp-auto/page-update`, including object authorization and the documented best-effort final timestamp check. The explicit allowlist may then grow from ten to twelve.
...
The authoritative frozen contract is `docs/PHASE_1_3_MUTATION_CONTRACTS.md`; the accepted mutation safety decision is `docs/ADR-002-MUTATION-SAFETY.md`. Phase 1.3.0 and Phase 1.3.1 are complete, Phase 1.3.2.0 is the next approved checkpoint, Phase 1.3.2 implementation is blocked pending that amendment, Phase 1.3 is not complete, and only the two validated Create Draft tools are currently exposed.
