...
3. **Phase 1.4.2 - Authenticated Image Upload (complete when the implementation lands on `main`):** implements one bounded Base64 image upload, optional authorized draft parent association, persistent idempotency, Core validation, private bounded audit, uninstall cleanup, and the fifteen-tool allowlist.
