...
4. **Phase 1.6.2 - Draft SEO Update (in progress; authorized):** expose only `wp-auto/seo-update` for authorized Post/Page drafts, including state-token checks, invariant verification, bounded attribution, and uninstall cleanup; the runtime becomes exactly twenty-three tools.
