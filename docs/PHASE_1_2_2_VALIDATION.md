...
- Object permission: `current_user_can( 'read_post', $post_id )`; password-protected posts additionally require `current_user_can( 'edit_post', $post_id )`
...
The service uses fixed raw query chunks of at most 100 posts with `no_found_rows = true`, `perm = readable`, and no unlimited query. Public ordering maps only to `date`, `modified`, `title`, or `ID`, with `ID` as a deterministic tie-breaker when needed.
...
Every candidate then passes the shared final eligibility rule: `current_user_can( 'read_post', $id )`, plus `current_user_can( 'edit_post', $id )` when `post_password` is non-empty. Search returns only `id`, `title`, `slug`, `status`, `link`, `author_id`, `date_gmt`, and `modified_gmt`. It returns no content, excerpt, password, metadata, totals, or total pages.

### `has_more`

The logical offset is `(page - 1) * per_page`, rejected before multiplication if it would overflow. It is applied only after each raw candidate passes final object eligibility. The scanner reads at most 100 raw posts per chunk, examines at most 1,000 raw candidates across at most ten queries, skips the requested count of authorized records, and then collects at most `per_page + 1` authorized records. `has_more` is true only when that extra authorized record exists.

If a deep page requires more than the 1,000-candidate window even when every candidate is authorized, it is rejected before querying. If interleaved authorization failures exhaust the scan window before the requested page can be proven, the service returns `WP_Error( 'wp_auto_pagination_window_exceeded', 'The requested page exceeds the supported search window.', array( 'status' => 400 ) )`. It never returns a shifted page or an unproven `has_more`. The 1,000-candidate bound supports ordinary interactive MCP navigation while limiting each request to ten bounded database queries.

## Post-get contract and errors

Post Get accepts exactly one integer `id` greater than zero. It retrieves through `get_post()`, verifies `post_type === 'post'`, and applies the same final eligibility rule as Search. A normal post requires `read_post`; a password-protected post requires both `read_post` and `edit_post`. The contract accepts no password input and exposes no password field. Only after authorization does Get return stored content. The exact output is `id`, `type`, `status`, `slug`, `title`, `excerpt`, `content`, `link`, `author_id`, `date_gmt`, `modified_gmt`, `featured_media_id`, `categories`, and `tags`.
...
- PHPUnit: 58 tests, 315 assertions passed.
- WordPress Coding Standards: 22 files passed.

Coverage includes both strict schemas, server defaults, all allowed enums, rejected statuses/orderings/excess properties, search length, page bounds, `per_page` 1/50/51, offset overflow, pre-query deep-page rejection, the ten-query raw scan cap, fixed query arguments, no unlimited query, exact outputs, stored dates/content, taxonomy errors, ability entry permission, published/own/other/private/pending/future visibility, shared Search/Get eligibility, password-protected posts with and without object edit permission, existence hiding, logical pages after a leading rejection, multiple interleaved authorization failures, permission-safe `has_more`, deterministic ordering, Core category registration, production class loading, and the exact four-ability allowlist.
...
| Permission-aware pagination | A disposable `map_meta_cap` rule denied `read_post` for A only; pages 1 and 2 returned B and C with `has_more` true then false |
| Search/Get consistency | The `map_meta_cap`-hidden A was absent from Search and Get returned the generic existence-hiding error |
| Password-protected post, subscriber | Search returned no matching item and Get returned the generic existence-hiding error |
| Password-protected post, editor | Search and Get succeeded through object-level `edit_post`; content was returned without a password field |
...
- `composer test`: passed; 58 tests and 315 assertions.
- `composer lint`: passed; 22 files checked.
- `composer audit --locked`: passed; no security vulnerability advisories found.
- `git diff --check`: passed.

Plugin Check was not preinstalled in this disposable environment. Downloading and executing a remote plugin solely for this run was rejected by the execution safety policy, so Plugin Check remains an explicit pre-release check rather than a claimed pass.
