...
- SEC-2 and SEC-3 require the ADR-003 atomic ownership amendment. Do not use `add_option()` as an atomic ownership primitive. Do not introduce arbitrary direct SQL. Only the ADR-003-approved ownership operations may use the narrow `$wpdb->options` SQL exception after the amendment lands on `main`.
