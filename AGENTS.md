...
Phase 1.3.0, Phase 1.3.1, and Phase 1.3.2.0 are formally sealed. Phase 1.3.2 is implemented, merged, validated on `main`, and formally sealed with an exact twelve-tool Direct MCP runtime. SEC-1 is resolved on `main`. Phase 1.3.3 SEC-2 remains open because the prior `add_option()` mutex candidate was rejected, and SEC-3 is a confirmed concurrent Create initial-claim blocker. The Atomic Ownership Architecture Amendment is approved; runtime SEC-2/SEC-3 remediation and the broader Phase 1.3.3 audit remain paused until ADR-003 is landed on `main`; Phase 1.3.4 remains blocked. Do not implement publishing, deletion, media, taxonomy mutation, SEO, Cloud, or later roadmap work unless an explicit later task authorizes it.
...
- Create uses concurrent-safe persistent idempotency scoped to site, actor, Ability, and key. The prior `add_option()` atomic-uniqueness assumption is invalidated by SEC-3. Until ADR-003 is landed on `main`, no runtime remediation may proceed. The approved amendment will use a strict insert-if-absent ownership primitive against a private non-autoloaded `$wpdb->options` row; never release an uncertain claim before deterministic resolution proves that doing so cannot duplicate an object.
- SEC-2 and SEC-3 require the ADR-003 atomic ownership amendment. Do not use `add_option()` as an atomic ownership primitive. Do not introduce arbitrary direct SQL. ACTIVE RUNTIME: direct SQL remains limited to the ADR-003-approved AtomicOwnershipStore ownership operations. EXPLICIT WORDPRESS UNINSTALL: the only additional direct-SQL authority is ADR-004, limited to its three approved read-only SQL families: (1) current-blog options enumeration/final verification; (2) current-blog postmeta exact-key audit-meta verification; and (3) multisite-only `$wpdb->blogs` `blog_id` keyset enumeration. ADR-004 authorizes no direct SQL write and creates no active-runtime SQL authority.
