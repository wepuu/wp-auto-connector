...
- Mutation tools must implement idempotency and optimistic concurrency as specified in the Phase 1 plan.
...
- SEC-2 and SEC-3 are resolved by the landed ADR-003 atomic ownership runtime. Do not introduce arbitrary direct SQL. ACTIVE RUNTIME: direct SQL remains limited to the ADR-003-approved `AtomicOwnershipStore` ownership operations. EXPLICIT WORDPRESS UNINSTALL: the only additional direct-SQL authority is ADR-004, limited to its four approved read-only SQL families: (1) current-blog options enumeration/final verification; (2) current-blog postmeta exact-key audit-meta verification; (3) current-blog termmeta exact-key taxonomy-audit verification; and (4) multisite-only `$wpdb->blogs` `blog_id` keyset enumeration. ADR-004 authorizes no direct SQL write and creates no active-runtime SQL authority.
...
## Phase 1.5 taxonomy invariants

- `docs/PHASE_1_5_TAXONOMY_CONTRACTS.md` is the authoritative public contract; `docs/ADR-006-TAXONOMY-SAFETY.md` records the safety decisions.
- Phase 1.5.0 was documentation-only and kept the exact eighteen-tool runtime. The authorized Phase 1.5.1 Category Create and Phase 1.5.2 Tag Create checkpoints add exactly two tools; assignment remains unimplemented and may enter the allowlist only at its named checkpoint.
- Only the built-in `category` and `post_tag` taxonomies are supported. Custom taxonomies, term update/delete, arbitrary term meta, implicit term creation, and generic taxonomy APIs remain unavailable.
- Category/Tag Create require the fixed taxonomy object's actual `cap->manage_terms` at Ability and service layers and use persistent ADR-003 ownership scoped to site, actor, Ability, and key. Existing terms are conflicts, not implicit idempotent replays.
- Assignment is limited to an authorized built-in Post in `draft` status. It requires the taxonomy's actual `cap->assign_terms`, the fixed Post type edit baseline, and final target `edit_post`.
- Assignment accepts only one fixed taxonomy and bounded unique term IDs, rejects current selected/protected sets above 50 through a bounded 51-item probe, uses exact replacement with a required `expected_term_ids` precondition, forbids an empty desired set, and reports its best-effort non-CAS concurrency limit.
- Assignment must verify the selected final set, the non-target approved taxonomy, and protected Post invariants after Core writes. It is destructive because exact replacement can remove relationships, though it cannot delete terms.
