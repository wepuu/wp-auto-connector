...
Phase 1.3.0 mutation contracts are frozen and Phase 1.3.1 provides authenticated Post/Page Create Draft tools: `wp-auto-post-create-draft` and `wp-auto-page-create-draft`. Create operations always produce drafts owned by the authenticated user and use persistent idempotency claims. The Phase 1.3.2.0 `modified_gmt` compatibility contract is sealed on `main`. The Phase 1.3.2 feature branch adds locally validated draft-only Post/Page Update tools for an exact twelve-tool surface; the implementation has passed review and remains pending PR, CI, merge, and formal sealing.

No publish, delete, arbitrary content write, cloud, telemetry, or automation operation is included. The scoped mutation boundary is limited to Create Draft and the reviewed, not-yet-merged Draft Update implementation.
