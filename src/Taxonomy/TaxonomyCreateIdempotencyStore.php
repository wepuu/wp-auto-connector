<?php
/**
 * Persistent taxonomy Create idempotency claims.
 *
...
/**
 * Stores one durable, non-autoloaded option per taxonomy Create scope.
 */
final class TaxonomyCreateIdempotencyStore {
	private const OPTION_PREFIX = 'wp_auto_connector_taxonomy_idempotency_';
	...
	/**
	 * Atomically claim a scope or return its existing record.
	 *
	...
	/**
	 * Persist the known target term ID while retaining ownership.
	 *
	 * @param string               $option_name Option name.
	 * @param array<string, mixed> $record Existing record.
	 * @param int                  $target_id Created term ID.
	 */
	...
	/**
	 * Release a claim only after Core proved that no term was created.
	 *
	 * @param string               $option_name Option name.
	...
	private function is_valid_persisted_record( array $record ): bool {
		...
		$target_valid = 'in_progress' === $record['state'] || $record['target_id'] >= 1;

		return $keys === $expected
			&& 1 === $record['version']
			&& is_int( $record['actor_user_id'] )
			&& $record['actor_user_id'] >= 1
			&& in_array( $record['ability'], array( 'wp-auto/category-create', 'wp-auto/tag-create' ), true )
	...
	/**
	 * Update a record and verify the stored value.
	 *
	 * @param string               $option_name Option name.
