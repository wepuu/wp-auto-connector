<?php
/**
 * Category Create ability registration tests.
 *
...
/** Covers Category Create registration and capability boundaries. */
final class CategoryCreateAbilityTest extends TestCase {
	...
	public function test_registers_on_the_abilities_api_hook(): void {
		$ability = new CategoryCreateAbility();
		$ability->register();

		self::assertSame( 'wp-auto/category-create', CategoryCreateAbility::NAME );
	...
	public function test_registration_has_category_contract_and_mutation_annotations(): void {
		( new CategoryCreateAbility() )->register_ability();

		$registration = $GLOBALS['wp_auto_test_registered_ability'];
		$args         = $registration['args'];

		self::assertSame( CategoryCreateAbility::NAME, $registration['name'] );
		self::assertSame( TaxonomyAbilityCategory::SLUG, $args['category'] );
		self::assertSame( array( 'name', 'idempotency_key', 'slug', 'description', 'parent_id' ), array_keys( $args['input_schema']['properties'] ) );
	...
	/** Permission uses the built-in Category taxonomy's actual manage_terms capability. */
	public function test_permission_requires_manage_terms(): void {
		...
		$GLOBALS['wp_auto_test_taxonomies']['category']->cap->manage_terms = 'manage_custom_categories';
		self::assertFalse( $ability->check_permission() );
		$GLOBALS['wp_auto_test_capabilities']['manage_custom_categories'] = true;
		self::assertTrue( $ability->check_permission() );
	}
}
