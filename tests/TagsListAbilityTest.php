...
final class TagsListAbilityTest extends TestCase {
	...
	public function test_registration_has_the_frozen_contract(): void {
		...
		$output_item  = $output['properties']['items']['items'];
		...
		self::assertSame( array( 'id', 'name', 'slug', 'description', 'count' ), array_keys( $output_item['properties'] ) );
		self::assertSame( array( 'id', 'name', 'slug', 'description', 'count' ), $output_item['required'] );
		self::assertSame( array( 'integer', 'string', 'string', 'string', 'integer' ), array_column( $output_item['properties'], 'type' ) );
